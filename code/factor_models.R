...
#https://github.com/crweber9874/electoral_contestation/tree/paper_v3
...
full_data_key = c(             "trust_congress" , "trust_president", 
                               "trust_sc",
                               "trust_stateleg", 
                               "trust_governor"
                               ) 
...
with(df, cor(cbind(trust_stateleg , trust_governor))) # 0.76
...
factor_model<-fa(dat, fm = 'minres', nfactors = 6)
...
rownames(x) = c("Protest_Election", "Burn", "Court", "Recount", "Criticize",
                ...
                "Concern(Accept)", "Concern(Delay)", "Concern(Illegal)",
                "Yard Sign", "Volunteer",
...
plot2<- ggplot(gplot, aes(x=factor, y=eigenvalues,
  ...
  theme(plot.title=element_text(face="bold", vjust=2,colour="#3C3C3C",size=11)) +
  theme(axis.text.x=element_text(size=11,colour="#535353",face="bold")) +
  theme(axis.text.y=element_text(size=11,colour="#535353",face="bold")) +
  theme(axis.title.y=element_text(size=11,colour="#535353",face="bold",vjust=1.5)) +
  theme(axis.title.x=element_text(size=11,colour="#535353",face="bold",vjust=-.5))+
  scale_x_continuous("Factors", limits=c(0,20))+
  scale_y_continuous("Eigenvalues", limits=c(-2,4))
plot2

dev.copy(png,'section2_1.jpg',
         width = 750, height = 500,)
dev.off()
##### Now estimate a bunch of factor models to explore uniqueness#####
full_data_key = c(            "violent", "burn", "court", "recount", "criticize",
                              "trust_congress" , "trust_president", "trust_sc",
                              "trust_governor", "trust_stateleg",
                              "concern_covid", "concern_lines", "concern_intimidate",
...
out = cfa(model1a, ordered=full_data_key, data=df[,full_data_key]) 
semPaths(out, 
         what="paths", "est", style="lisrel", 
         layout = "circle2", rotation=1,
thresholds=FALSE, residuals=FALSE, intercepts=FALSE, 
sizeMan2=1, sizeMan=7, sizeLat2=7, sizeLat=6, 
label.prop=1, label.cex=1, title=TRUE, node.width=1,
edge.label.cex = 0.5, 
nCharNodes=0, label.font=10, label.scale=FALSE)


dev.copy(png,'section2_3.jpg',
         width = 750, height = 500,)
dev.off()

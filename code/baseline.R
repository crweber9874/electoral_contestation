...
plot_dat =  subset(plot_dat, variable != c("Colorado*", "New Mexico*", "Nevada*", "Utah*"))

ggplot(plot_dat, aes(y=mid, ymin=min, ymax=max, x=variable)) + 
  facet_wrap (~item)+
  geom_errorbar( width = 0.01, alpha = 0.9,  position=position_dodge(width = 0.2), size =1.2, colour = "grey", alpha = 0.3) +
  geom_point(size = 1.8, position=position_dodge(width = 0.2), alpha = 0.3, colour = "black") +
  scale_fill_grey() +
  theme_bw() + 
...
dat$soft <- rowMeans(cbind(dat$recount, dat$criticize, dat$court), na.rm = T) %>% zero.one()
dat$hard <- rowMeans(cbind(dat$burn, dat$violent), na.rm = T) %>% zero.one()
...
plot_dat = rbind(winner_loser_baseline_l(dat, "hard"),
  ...
  mutate(item = rep(c("High Cost", "Low Cost"), each =17)) 
